...
export class HevySyncService {
  ...
  private async syncWorkout(hevyWorkout: HevyWorkout) {
    // Check if workout already exists
    const existing = await prisma.workout.findUnique({
      where: { id: hevyWorkout.id }
    })
    ...
    const workout = await prisma.workout.create({
      data: {
        ...
        startTime: new Date(hevyWorkout.start_time),
        endTime: new Date(hevyWorkout.end_time),
        comment: hevyWorkout.comment,
        isTemplate: hevyWorkout.is_template,
        source: 'HEVY',
        sourceId: hevyWorkout.id,
  ...
  private async syncExercise(workoutId: string, hevyExercise: HevyExercise) {
    ...
    let exercise = await prisma.exercise.findUnique({
      where: { id: hevyExercise.exercise_template_id }
    })
    ...
    if (!exercise) {
      exercise = await prisma.exercise.create({
        data: {
          id: hevyExercise.exercise_template_id,
          name: hevyExercise.exercise_template.name,
          description: hevyExercise.exercise_template.description,
          source: 'HEVY',
    ...
    for (const hevySet of hevyExercise.sets) {
      await this.syncSet(workoutId, exercise.id, hevySet)
    }
  }

  private async syncSet(workoutId: string, exerciseId: string, hevySet: HevySet) {
